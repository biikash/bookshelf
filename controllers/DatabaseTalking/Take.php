...
class Take extends Talking
{
    ...
    public function takeorder (Orders $order,&$orderNumber=null):bool
    {
        try{
           ...
           $this->stat=$this->conn->prepare('INSERT INTO `bookstore`.`address` VALUES (?,?,?,?,?)');
           ...
           $this->stat=$this->conn->prepare('INSERT INTO `bookstore`.`customer` (customer_id, name, address_id, email, phone, password) VALUES (?,?,?,?,?,?)');
           $this->stat->execute(array(
               0=>$customerId,
               1=>$customer->getName(),
               2=>$addressId,
           ...
           /**
             ...
             */
           $this->stat=$this->conn->prepare('INSERT INTO `bookstore`.`orders` (order_id, customer_id) values (?,?)');
           $this->stat->execute(array(
               0=>$orderId,
               1=>$customerId
           ));
    ...
    private function insertANOrderDetails($orderId,OrderDetails $detail){
        try{
            $this->stat = $this->conn->prepare('INSERT INTO `bookstore`.`order_details` (order_details_id, order_id, book_id,quantity) VALUES (?,?,?,?)');
            $this->stat->execute(array(
                0 => rand(1, 1000000000),
                1 => $orderId,
            ...
            UPDATE `bookstore`.`book` set quantity=(Select quantity from `bookstore`.`book` where book_id=?)-? where book_id=?");
            $this->stat->execute(array(
